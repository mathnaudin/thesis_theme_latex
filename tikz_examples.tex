...
\begin{verbatim}
  ...
  \draw[->,=>latex] (O) -- (Ox);
  \draw[->,=>latex] (O) -- (Oy);
  % traçage de la fonction e(t) = E, t < T
  \draw (E) -- (e_T);           % valeur de e(t)
  \draw[dashed] (e_T) -- (T); % pointillés à t = T
  % ajout des notations
  \draw (E) node[left]{$E$};
  \draw (O) node[left,below]{$0$};
  \draw (Oy) node[left]{$e(t)$};
  \draw (Ox) node[below]{$t$};
  \draw (T) node[below]{$T$};
\end{verbatim}
...
\newpage
\subsection{Graphiques avec barres d'erreur}
Crédits :\\
